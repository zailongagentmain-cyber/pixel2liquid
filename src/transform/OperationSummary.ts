...
import { PageStructure, ProductMap, AssetMapping, OperationSummary, OperationItem } from './types.js';
...
export class OperationSummaryGenerator {
  ...
  async generate(
    ...
    const productsToCreate: OperationItem[] = missingProducts.map(p => ({
    ...
    const assetsToUploadItems: OperationItem[] = assetsToUpload.map(a => ({
      type: 'upload_asset' as const,
      handle: a.filename,
      filename: a.filename,
   ...
   * 把 handle 转换成可读标题
   */
  private handleToTitle(handle: string): string {
    return handle
      .split('-')
