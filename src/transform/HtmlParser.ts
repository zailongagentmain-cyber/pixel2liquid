...
import { PageStructure, BlockMap, AssetMapping } from './types.js';
...
export class HtmlParser {
  ...
  detectPageType(url: string, html: string): 'product' | 'collection' | 'home' | 'blog' {
    ...
    // 特殊页面检测
    if ($('[data-section-type="product"]').length > 0) return 'product';
    if ($('[data-section-type="collection"]').length > 0) return 'collection';
    if ($('.shopify-section--highlighted').length > 0) return 'home';
  ...
  extractProductBlocks(html: string): BlockMap[] {
    ...
    $('.product__title h1, .product-title, h1[class*="product"]').each((_, el) => {
      blocks.push({
        sourceSelector: this.getSelector($, el as any),
   ...
   * 生成唯一选择器
   */
  private getSelector($: cheerio.CheerioAPI, el: any): string {
    const node = $(el);
