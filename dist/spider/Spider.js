...
export class Spider {
    ...
    async crawl() {
        ...
        const siteMap = {
            ...
            totalAssets: collectedPages.reduce((sum, p) => sum + p.images.length + p.fonts.length + p.js.length, 0),
        };
        // 保存站点地图
    ...
    async phase1QuickScan() {
        ...
        await fse.writeJson(path.join(this.options.outputDir, 'quick-scan.json'), scanResult, { spaces: 2 });
     ...
     */
    async phase2ProgressiveCollect(pages) {
        const collected = [];
        ...
        if (failedUrls.length > 0) {
            await fse.writeJson(path.join(this.options.outputDir, 'errors.json'), { failedUrls, timestamp: new Date().toISOString() }, { spaces: 2 });
        }
        return collected;
    }
    /**
     ...
     */
    async collectSinglePage(page, browser, context) {
        ...
        try {
            ...
            if (this.options.asyncMode && this.resourceQueue) {
                ...
                await fse.writeFile(path.join(this.options.outputDir, page.localPath), processedHtml, 'utf-8');
                // 释放大字符串内存
                ...
                // 收集资源URL（仅记录，不下载）
                const assetUrls = this.collectAssetUrls(html);
                page.images = assetUrls.filter(a => a.type === 'image').map(a => ({ url: a.url, localPath: a.url, type: 'image' }));
                page.fonts = assetUrls.filter(a => a.type === 'font').map(a => ({ url: a.url, localPath: a.url, type: 'font' }));
                page.js = assetUrls.filter(a => a.type === 'js').map(a => ({ url: a.url, localPath: a.url, type: 'js' }));
                console.log(`    📄 静态采集: ${assetUrls.length} 个资源URL (保留原URL)`);
                // 保存原始 HTML（不做任何路径替换）
                const pageDir = path.dirname(page.localPath);
                await fse.ensureDir(path.join(this.options.outputDir, pageDir));
                await fse.writeFile(path.join(this.options.outputDir, page.localPath), html, 'utf-8');
                // 释放大字符串内存
                page.html = html;
            }
            else {
                ...
                const pageDir = path.dirname(page.localPath);
                await fse.ensureDir(path.join(this.options.outputDir, pageDir));
                await fse.writeFile(path.join(this.options.outputDir, page.localPath), resolvedHtml, 'utf-8');
                // 释放大字符串内存
     ...
     */
    replaceUrlsWithProxy(html, assets) {
        if (!this.proxyServer)
            return html;
        let result = html;
