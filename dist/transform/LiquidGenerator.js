...
`;
     ...
     */
    generateCollectionTemplate(page) {
...
`;
     ...
     */
    generateProductJson(page, productMap) {
...
{% endschema %}`;
  ...
  {% endform %}
     ...
     * 生成首页区块
     */
    generateHomeBlocks(blocks) {
        return blocks.map(b => `<!-- ${b.dataType}: ${b.sourceSelector} -->`).join('\n    ');
    }
    /**
     * 替换 HTML 中的 CDN URL 为 Liquid filter
     */
    replaceCdnWithLiquid(html, assetMappings) {
        let result = html;
        for (const mapping of assetMappings) {
            if (mapping.targetAssetUrl) {
                // 替换 CDN URL 为 Liquid filter
                result = result.replace(new RegExp(this.escapeRegex(mapping.cdnUrl), 'g'), mapping.liquidReference.replace(/'/g, '"'));
            }
        }
        return result;
    }
...
//# sourceMappingURL=LiquidGenerator.js.map